...
// Usamos un esquema completo con expediente y diente por defecto
const PatientSchema = z.object({
  _id: z.string().optional(),          // si viene, actualiza; si no, crea
  name: z.string().min(1),
  recordNumber: z.string().min(1),     // número de expediente
  defaultTooth: z.string().optional().default(''), // diente sugerido
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const col = db.collection('patients'); // sin genérico para evitar choques de tipos
  ...
  if (req.method === 'GET') {
    ...
    const items = raw.map((d) => ({
      ...
      name: String(d.name ?? ''),
      recordNumber: String(d.recordNumber ?? ''),
      defaultTooth: String(d.defaultTooth ?? ''),
        ...
        await col.updateOne(
          { _id },
          {
            $set: {
              ...
              defaultTooth: parsed.defaultTooth ?? '',
